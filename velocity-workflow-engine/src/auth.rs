...
impl AuthManager {
    ...
    pub fn add_role(&self, role: Role) {
        self.roles.lock().unwrap().insert(role.name.clone(), role);
    }

    pub fn authorize(&self, claims: &Claims, permission: &Permission) -> bool {
        if self.denied.lock().unwrap().contains(&claims.subject) {
            return false;
        }
        ...
        for role_name in &claims.roles {
            if let Some(role) = roles.get(role_name) {
                if role.permissions.contains(permission)
                    || role.permissions.contains(&Permission::AdminAccess)
                {
                    return true;
                }
            }
    ...
    pub fn deny_subject(&self, subject: &str) {
        self.denied.lock().unwrap().insert(subject.to_string());
    }
