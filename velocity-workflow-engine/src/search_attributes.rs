...
impl SearchAttributeValue {
    ...
    pub fn as_double(&self) -> Option<f64> {
        match self { Self::Double(v) => Some(*v), _ => None }
    }
    ...
    pub fn as_datetime(&self) -> Option<i64> {
        match self { Self::Datetime(v) => Some(*v), _ => None }
    }

    pub fn as_keyword_list(&self) -> Option<&[String]> {
        match self { Self::KeywordList(v) => Some(v), _ => None }
    }
}
...
impl SearchAttributeDefinition {
    ...
    fn register_system_attributes(&self) {
        let system_attrs = vec![
            ...
            ("WorkflowType", SearchAttributeType::Keyword, "Workflow type name"),
            ("StartTime", SearchAttributeType::Datetime, "Workflow start time"),
            ("CloseTime", SearchAttributeType::Datetime, "Workflow close time"),
            ("ExecutionStatus", SearchAttributeType::Keyword, "Workflow execution status"),
            ("ExecutionDuration", SearchAttributeType::Int, "Workflow execution duration in nanos"),
            ("StateTransitionCount", SearchAttributeType::Int, "State transition count"),
            ("HistoryLength", SearchAttributeType::Int, "History event count"),
            ...
            ("TemporalChangeVersion", SearchAttributeType::KeywordList, "Change version list"),
            ("BatchOperationId", SearchAttributeType::Keyword, "Batch operation ID"),
            ("ParentWorkflowId", SearchAttributeType::Keyword, "Parent workflow ID"),
            ("ParentRunId", SearchAttributeType::Keyword, "Parent run ID"),
            ("RootWorkflowId", SearchAttributeType::Keyword, "Root workflow ID"),
            ("RootRunId", SearchAttributeType::Keyword, "Root run ID"),
            ("BinaryChecksums", SearchAttributeType::KeywordList, "Binary checksums"),
            ("HistorySizeBytes", SearchAttributeType::Int, "History size in bytes"),
    ...
    pub fn add_custom_attribute(&self, name: &str, sa_type: SearchAttributeType) -> Result<(), SearchAttributeError> {
        let mut fields = self.fields.write().unwrap();
    ...
    pub fn remove_custom_attribute(&self, name: &str) -> Result<(), SearchAttributeError> {
        let mut fields = self.fields.write().unwrap();
        let field = fields.get(name).ok_or(SearchAttributeError::NotFound(name.to_string()))?;
        if field.is_system {
    ...
    pub fn add_alias(&self, alias: &str, target: &str) -> Result<(), SearchAttributeError> {
        ...
        self.aliases.write().unwrap().insert(alias.to_string(), target.to_string());
        Ok(())
    }

    pub fn validate(&self, name: &str, value: &SearchAttributeValue) -> Result<(), SearchAttributeError> {
        ...
        let field = fields.get(&resolved).ok_or(SearchAttributeError::NotFound(name.to_string()))?;
    ...
    pub fn validate_attributes(&self, attrs: &HashMap<String, SearchAttributeValue>) -> Result<(), SearchAttributeError> {
    ...
    pub fn list_system_fields(&self) -> Vec<SearchAttributeField> {
        self.fields.read().unwrap().values()
...
impl SearchAttributeMapper {
    ...
    pub fn add_mapping(&self, field_name: &str, db_column: &str) {
        self.field_to_db_column.write().unwrap().insert(field_name.to_string(), db_column.to_string());
        self.db_column_to_field.write().unwrap().insert(db_column.to_string(), field_name.to_string());
    }

    pub fn field_to_column(&self, field_name: &str) -> Option<String> {
        self.field_to_db_column.read().unwrap().get(field_name).cloned()
    }

    pub fn column_to_field(&self, db_column: &str) -> Option<String> {
        self.db_column_to_field.read().unwrap().get(db_column).cloned()
    }

    pub fn map_attributes_to_columns(&self, attrs: &HashMap<String, SearchAttributeValue>) -> HashMap<String, SearchAttributeValue> {
    ...
    pub fn map_columns_to_attributes(&self, columns: &HashMap<String, SearchAttributeValue>) -> HashMap<String, SearchAttributeValue> {
        let mapping = self.db_column_to_field.read().unwrap();
        columns.iter().map(|(k, v)| {
...
impl std::fmt::Display for SearchAttributeError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::TypeMismatch { field, expected, got } => write!(f, "type mismatch for {}: expected {}, got {}", field, expected, got),
            Self::InvalidValue { field, message } => write!(f, "invalid value for {}: {}", field, message),
        }
    }
}
...
mod tests {
    ...
    fn test_add_custom_attribute() {
        let def = SearchAttributeDefinition::new();
        def.add_custom_attribute("CustomField", SearchAttributeType::Text).unwrap();
    ...
    fn test_add_duplicate_custom() {
        let def = SearchAttributeDefinition::new();
        def.add_custom_attribute("MyField", SearchAttributeType::Int).unwrap();
        let err = def.add_custom_attribute("MyField", SearchAttributeType::Text).unwrap_err();
    ...
    fn test_reserved_name() {
        let def = SearchAttributeDefinition::new();
        let err = def.add_custom_attribute("TemporalCustom", SearchAttributeType::Text).unwrap_err();
    ...
    fn test_validate_correct_type() {
        let def = SearchAttributeDefinition::new();
        def.validate("WorkflowId", &SearchAttributeValue::Keyword("wf-1".to_string())).unwrap();
    }

    #[test]
    fn test_validate_wrong_type() {
        let def = SearchAttributeDefinition::new();
        let err = def.validate("WorkflowId", &SearchAttributeValue::Int(42)).unwrap_err();
    ...
    fn test_validate_not_found() {
        let def = SearchAttributeDefinition::new();
        let err = def.validate("NonExistent", &SearchAttributeValue::Text("x".to_string())).unwrap_err();
    ...
    fn test_remove_custom_attribute() {
        let def = SearchAttributeDefinition::new();
        def.add_custom_attribute("Temp", SearchAttributeType::Bool).unwrap();
    ...
    fn test_mapper() {
        ...
        assert_eq!(mapper.field_to_column("WorkflowId"), Some("workflow_id".to_string()));
    ...
    fn test_mapper_batch_convert() {
        ...
        attrs.insert("WorkflowId".to_string(), SearchAttributeValue::Keyword("wf-1".to_string()));
        attrs.insert("CustomField".to_string(), SearchAttributeValue::Text("hello".to_string()));
    ...
    fn test_attribute_value_type_of() {
        assert_eq!(SearchAttributeValue::Text("x".into()).type_of(), SearchAttributeType::Text);
        assert_eq!(SearchAttributeValue::Int(42).type_of(), SearchAttributeType::Int);
        assert_eq!(SearchAttributeValue::Bool(true).type_of(), SearchAttributeType::Bool);
    }

    #[test]
    fn test_attribute_value_accessors() {
        assert_eq!(SearchAttributeValue::Text("hello".into()).as_text(), Some("hello"));
        ...
        assert_eq!(SearchAttributeValue::Datetime(12345).as_datetime(), Some(12345));
    }
    ...
    fn test_validate_batch() {
        ...
        let mut attrs = HashMap::new();
        attrs.insert("WorkflowId".to_string(), SearchAttributeValue::Keyword("wf-1".to_string()));
        attrs.insert("RunId".to_string(), SearchAttributeValue::Keyword("run-1".to_string()));
        def.validate_attributes(&attrs).unwrap();
    }

    #[test]
    fn test_search_attribute_type_from_i32() {
        assert_eq!(SearchAttributeType::from_i32(1), Some(SearchAttributeType::Text));
        assert_eq!(SearchAttributeType::from_i32(7), Some(SearchAttributeType::KeywordList));
