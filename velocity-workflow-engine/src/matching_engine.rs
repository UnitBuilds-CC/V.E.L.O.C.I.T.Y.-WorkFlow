...
impl TaskQueueId {
    pub fn new(ns: &str, name: &str, kind: TaskQueueKind, qt: TaskQueueType) -> Self {
        Self { namespace_id: ns.into(), name: name.into(), kind, queue_type: qt }
    }
    pub fn key(&self) -> String { format!("{}/{}/{:?}/{:?}", self.namespace_id, self.name, self.kind, self.queue_type) }
}
...
impl Default for VersionedData {
    fn default() -> Self { Self { current_version: 0, version_branches: Vec::new() } }
}
...
pub struct TaskQueueStats {
    pub tasks_added: AtomicU64, pub tasks_matched: AtomicU64,
    pub tasks_forwarded: AtomicU64, pub poll_count: AtomicU64,
    pub sync_match_count: AtomicU64, pub expired_tasks: AtomicU64,
}

impl TaskQueue {
    pub fn new(id: TaskQueueId) -> Self {
        Self { id, tasks: RwLock::new(VecDeque::new()), pollers: RwLock::new(VecDeque::new()), range_id: AtomicI64::new(1), ack_level: AtomicI64::new(0), stats: TaskQueueStats::default(), version_data: RwLock::new(VersionedData::default()) }
    }
    ...
    pub fn remove_poller(&self, poller_id: &str) {
        self.pollers.write().unwrap().retain(|p| p.poller_id != poller_id);
    }

    pub fn pending_count(&self) -> usize { self.tasks.read().unwrap().len() }
    pub fn poller_count(&self) -> usize { self.pollers.read().unwrap().len() }

    pub fn set_version(&self, version: i64, build_id: &str) {
        let mut vd = self.version_data.write().unwrap();
        vd.current_version = version;
        vd.version_branches.push(VersionBranch { version, build_id: build_id.into(), is_default: true });
    }
}
...
pub struct PartitionManagerStats {
    pub tasks_routed: AtomicU64, pub forward_count: AtomicU64,
}
...
impl PartitionManager {
    pub fn new(root_id: TaskQueueId, num_partitions: u32) -> Self {
        ...
        for i in 0..num_partitions {
            let pid = TaskQueueId { name: format!("{}__partition_{}", root.id.name, i), ..root.id.clone() };
            partitions.insert(i as i32, Arc::new(TaskQueue::new(pid)));
        }
        Self { num_partitions, partitions: RwLock::new(partitions), root_queue: root, stats: PartitionManagerStats::default() }
    }
    ...
    pub fn route_task(&self, task: MatchTask) -> i32 {
        ...
        if let Some(pq) = partitions.get(&partition) { pq.add_task(task); }
    ...
    pub fn poll_partition(&self, partition: i32, poller_id: &str) -> Option<MatchTask> {
        let partitions = self.partitions.read().unwrap();
        partitions.get(&partition).and_then(|pq| pq.match_task(poller_id))
    }
    ...
    pub fn total_pending(&self) -> usize {
        ...
        for (_, pq) in self.partitions.read().unwrap().iter() { total += pq.pending_count(); }
        total
    }

    pub fn partition_count(&self) -> usize { self.partitions.read().unwrap().len() }
}
...
impl Default for MatchingEngineConfig {
    fn default() -> Self {
        Self { num_partitions: 4, max_task_queue_idle_time: Duration::from_secs(60), forwarder_max_children: 10, sync_match_wait: Duration::from_millis(100), max_pollers_per_queue: 100 }
    }
}

#[derive(Debug, Default)]
pub struct MatchingEngineStats {
    pub queues_created: AtomicU64, pub tasks_added: AtomicU64,
    pub tasks_matched: AtomicU64, pub poll_count: AtomicU64,
}

impl MatchingEngine {
    pub fn new() -> Self {
        Self { task_queues: RwLock::new(HashMap::new()), partition_managers: RwLock::new(HashMap::new()), config: MatchingEngineConfig::default(), stats: MatchingEngineStats::default() }
    }

    pub fn get_or_create_queue(&self, id: &TaskQueueId) -> Arc<TaskQueue> {
        let key = id.key();
        if let Some(tq) = self.task_queues.read().unwrap().get(&key) { return tq.clone(); }
    ...
    pub fn queue_count(&self) -> usize { self.task_queues.read().unwrap().len() }

    pub fn total_pending(&self) -> usize {
        self.task_queues.read().unwrap().values().map(|tq| tq.pending_count()).sum()
    }
    ...
    pub fn health_report(&self) -> MatchingHealthReport {
        ...
        MatchingHealthReport { total_queues, total_pending, total_pollers, tasks_added: self.stats.tasks_added.load(Ordering::Relaxed), tasks_matched: self.stats.tasks_matched.load(Ordering::Relaxed) }
    }
}

#[derive(Debug)]
pub struct MatchingHealthReport {
    pub total_queues: usize, pub total_pending: usize, pub total_pollers: usize,
    pub tasks_added: u64, pub tasks_matched: u64,
}

fn fnv1a_hash(s: &str) -> u32 {
    let mut hash: u32 = 2166136261;
    for byte in s.bytes() { hash ^= byte as u32; hash = hash.wrapping_mul(16777619); }
    hash
}
...
mod tests {
    ...
    fn make_task_id(ns: &str, name: &str) -> TaskQueueId { TaskQueueId::new(ns, name, TaskQueueKind::Normal, TaskQueueType::Workflow) }

    fn make_task(id: i64, wf: &str) -> MatchTask {
        MatchTask { task_id: id, namespace_id: "ns".into(), workflow_id: wf.into(), run_id: "r".into(), task_type: TaskQueueType::Workflow, scheduled_time: now_millis(), priority: 0, forwarding_info: None, version: 0 }
    }
    ...
    fn test_task_queue_poller_management() {
        let tq = TaskQueue::new(make_task_id("ns", "tq-1"));
        tq.register_poller(PollerInfo { poller_id: "p1".into(), identity: "worker-1".into(), last_poll_time: now_millis(), rate_per_second: 10.0 });
        tq.register_poller(PollerInfo { poller_id: "p2".into(), identity: "worker-2".into(), last_poll_time: now_millis(), rate_per_second: 5.0 });
    ...
    fn test_matching_engine_health() {
        ...
        let id = make_task_id("ns", "tq-1");
        engine.add_task(&id, make_task(1, "wf-1"));
        engine.register_poller(&id, PollerInfo { poller_id: "p1".into(), identity: "w1".into(), last_poll_time: 0, rate_per_second: 1.0 });
    ...
    fn test_sync_match() {
        ...
        tq.register_poller(PollerInfo { poller_id: "p1".into(), identity: "w".into(), last_poll_time: 0, rate_per_second: 1.0 });
