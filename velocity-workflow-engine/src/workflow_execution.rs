...
impl WorkflowExecutionStatus {
    pub fn is_terminal(&self) -> bool { !matches!(self, WorkflowExecutionStatus::Running) }
    pub fn as_str(&self) -> &'static str {
        match self {
            Self::Running => "Running", Self::Completed => "Completed", Self::Failed => "Failed",
            Self::Cancelled => "Cancelled", Self::Terminated => "Terminated",
            Self::ContinuedAsNew => "ContinuedAsNew", Self::TimedOut => "TimedOut",
        }
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum WorkflowState { Created, Running, Completed, Zombie }

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum WorkflowStatus { Unknown, Running, Completed, Failed, Cancelled, Terminated, ContinuedAsNew, TimedOut }
...
impl MutableState {
    pub fn new(namespace_id: &str, workflow_id: &str, run_id: &str, workflow_type: &str, task_queue: &str) -> Self {
        Self {
            namespace_id: namespace_id.to_string(), workflow_id: workflow_id.to_string(),
            run_id: run_id.to_string(), state: RwLock::new(WorkflowState::Created),
            status: RwLock::new(WorkflowStatus::Running), workflow_type: workflow_type.to_string(),
            start_time: now_millis(), close_time: RwLock::new(None),
            task_queue: task_queue.to_string(), execution_timeout: None, run_timeout: None,
            task_timeout: Duration::from_secs(10),
            last_event_id: AtomicI64::new(0), last_first_event_id: AtomicI64::new(0),
            next_event_id: AtomicI64::new(1), version: AtomicI64::new(0),
            initiatied_id: AtomicI64::new(-1),
            activities: RwLock::new(HashMap::new()), timers: RwLock::new(HashMap::new()),
            child_workflows: RwLock::new(HashMap::new()), request_cancel: RwLock::new(HashMap::new()),
            signals: RwLock::new(VecDeque::new()), buffered_events: RwLock::new(VecDeque::new()),
            ...
            retry_policy: RwLock::new(None), retry_state: RwLock::new(RetryState::default()),
            search_attributes: RwLock::new(HashMap::new()), memo: RwLock::new(HashMap::new()),
    ...
    pub fn is_running(&self) -> bool { *self.state.read().unwrap() == WorkflowState::Running }

    // Activity management
    pub fn add_activity(&self, activity_id: &str, activity_type: &str, task_queue: &str) -> Result<(), String> {
        ...
        activities.insert(activity_id.to_string(), ActivityState {
            activity_id: activity_id.to_string(), activity_type: activity_type.to_string(),
            task_queue: task_queue.to_string(), state: ActivityStateEnum::Scheduled,
            scheduled_time: now_millis(), started_time: None, completed_time: None,
            attempt: 1, max_attempts: 3, last_failure: None, heartbeat_time: None,
            heartbeat_timeout: None, schedule_to_close_timeout: None,
            schedule_to_start_timeout: None, start_to_close_timeout: None,
            result: None,
        });
        self.stats.activities_tracked.fetch_add(1, Ordering::Relaxed);
        Ok(())
    }
    ...
    pub fn record_activity_completed(&self, activity_id: &str, result: Vec<u8>) -> Result<(), String> {
        let mut activities = self.activities.write().unwrap();
        let activity = activities.get_mut(activity_id).ok_or("Not found")?;
    ...
    pub fn pending_activities(&self) -> Vec<ActivityState> {
        self.activities.read().unwrap().values()
            .filter(|a| !matches!(a.state, ActivityStateEnum::Completed | ActivityStateEnum::Failed))
            .cloned().collect()
    }
    ...
    pub fn add_timer(&self, timer_id: &str, fire_time: i64) -> Result<(), String> {
        ...
        if timers.contains_key(timer_id) { return Err(format!("Timer {} exists", timer_id)); }
        timers.insert(timer_id.to_string(), TimerState {
            timer_id: timer_id.to_string(), fire_time, created_time: now_millis(),
            state: TimerStateEnum::Created, cancelled: false,
        });
    ...
    pub fn add_child_workflow(&self, child_id: &str, workflow_type: &str, namespace_id: &str) -> Result<(), String> {
        let mut children = self.child_workflows.write().unwrap();
        if children.contains_key(child_id) { return Err(format!("Child {} exists", child_id)); }
        children.insert(child_id.to_string(), ChildWorkflowState {
            child_id: child_id.to_string(), workflow_type: workflow_type.to_string(),
            namespace_id: namespace_id.to_string(), state: ChildState::Initiated,
            started_time: None, completed_time: None, result: None, error: None,
        });
    ...
    pub fn buffer_signal(&self, signal_name: &str, input: Vec<u8>, identity: &str) {
        ...
        signals.push_back(SignalInfo { signal_name: signal_name.to_string(), input, identity: identity.to_string(), received_at: now_millis() });
    ...
    fn record_transition(&self, new_state: &str) {
        ...
        self.state_transition_history.write().unwrap().record(new_state);
    }
    ...
    pub fn set_search_attribute(&self, key: &str, value: SearchAttributeValue) {
        self.search_attributes.write().unwrap().insert(key.to_string(), value);
    }
...
pub struct ActivityState {
    ...
    pub state: ActivityStateEnum, pub scheduled_time: i64,
    pub started_time: Option<i64>, pub completed_time: Option<i64>,
    pub attempt: u32, pub max_attempts: u32, pub last_failure: Option<String>,
    pub heartbeat_time: Option<i64>, pub heartbeat_timeout: Option<Duration>,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ActivityStateEnum { Scheduled, Started, Completed, Failed, Cancelled, TimedOut }

#[derive(Debug, Clone)]
pub struct TimerState {
    pub timer_id: String, pub fire_time: i64, pub created_time: i64,
    pub state: TimerStateEnum, pub cancelled: bool,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum TimerStateEnum { Created, Fired, Cancelled }

#[derive(Debug, Clone)]
pub struct ChildWorkflowState {
    pub child_id: String, pub workflow_type: String, pub namespace_id: String,
    pub state: ChildState, pub started_time: Option<i64>,
    pub completed_time: Option<i64>, pub result: Option<Vec<u8>>,
...
pub struct RequestCancelState {
    pub child_id: String, pub initiated_id: i64, pub cancel_requested: bool,
}

#[derive(Debug, Clone)]
pub struct SignalInfo {
    pub signal_name: String, pub input: Vec<u8>, pub identity: String, pub received_at: i64,
}

#[derive(Debug, Clone)]
pub struct HistoryEvent {
    pub event_id: i64, pub event_type: String, pub timestamp: i64,
    pub version: i64, pub task_id: i64, pub attributes: HashMap<String, Vec<u8>>,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum QueryState { Buffered, Unblocked, Completed, Failed }

#[derive(Debug, Clone)]
pub struct QueryEntry {
    pub query_id: String, pub query_type: String, pub query_args: Vec<u8>,
    pub state: QueryState, pub result: Option<Vec<u8>>, pub error: Option<String>,
    pub received_at: i64, pub completed_at: Option<i64>,
}
...
impl QueryRegistry {
    pub fn new() -> Self {
        Self { queries: RwLock::new(HashMap::new()), buffered_count: AtomicU64::new(0), completed_count: AtomicU64::new(0) }
    }

    pub fn buffer_query(&self, query_id: &str, query_type: &str, args: Vec<u8>) -> Result<(), String> {
        let mut queries = self.queries.write().unwrap();
        if queries.contains_key(query_id) { return Err("Query already exists".into()); }
        queries.insert(query_id.to_string(), QueryEntry {
            query_id: query_id.to_string(), query_type: query_type.to_string(),
            query_args: args, state: QueryState::Buffered, result: None, error: None,
            received_at: now_millis(), completed_at: None,
        });
    ...
    pub fn buffered_queries(&self) -> Vec<QueryEntry> {
        self.queries.read().unwrap().values().filter(|q| q.state == QueryState::Buffered).cloned().collect()
    }

    pub fn pending_count(&self) -> usize {
        self.queries.read().unwrap().values().filter(|q| !matches!(q.state, QueryState::Completed | QueryState::Failed)).count()
    }
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum UpdateState { Admitted, Accepted, Completed, Rejected }

#[derive(Debug, Clone)]
pub struct UpdateEntry {
    pub update_id: String, pub update_name: String, pub args: Vec<u8>,
    pub state: UpdateState, pub result: Option<Vec<u8>>,
    pub rejection_reason: Option<String>,
    pub admitted_at: i64, pub accepted_at: Option<i64>, pub completed_at: Option<i64>,
}
...
pub struct UpdateRegistryStats {
    pub admitted: AtomicU64, pub accepted: AtomicU64,
    pub completed: AtomicU64, pub rejected: AtomicU64,
}

impl UpdateRegistry {
    pub fn new() -> Self {
        Self { updates: RwLock::new(HashMap::new()), stats: UpdateRegistryStats::default() }
    }

    pub fn admit_update(&self, update_id: &str, update_name: &str, args: Vec<u8>) -> Result<(), String> {
        let mut updates = self.updates.write().unwrap();
        if updates.contains_key(update_id) { return Err("Update exists".into()); }
        updates.insert(update_id.to_string(), UpdateEntry {
            update_id: update_id.to_string(), update_name: update_name.to_string(),
            args, state: UpdateState::Admitted, result: None, rejection_reason: None,
            admitted_at: now_millis(), accepted_at: None, completed_at: None,
        });
    ...
    pub fn pending_updates(&self) -> Vec<UpdateEntry> {
        self.updates.read().unwrap().values()
...
pub struct StateTransition {
    pub transition_id: u64, pub from_state: String, pub to_state: String,
    pub timestamp: i64, pub trigger: String,
}
...
impl StateTransitionHistory {
    pub fn new() -> Self {
        Self { transitions: VecDeque::new(), next_id: 1, current_state: "Created".to_string(), max_history: 1000 }
    }

    pub fn record(&mut self, to_state: &str) {
        let transition = StateTransition {
            transition_id: self.next_id, from_state: self.current_state.clone(),
            to_state: to_state.to_string(), timestamp: now_millis(), trigger: String::new(),
        };
        self.next_id += 1;
        self.current_state = to_state.to_string();
        self.transitions.push_back(transition);
        while self.transitions.len() > self.max_history { self.transitions.pop_front(); }
    }

    pub fn record_with_trigger(&mut self, to_state: &str, trigger: &str) {
        let transition = StateTransition {
            transition_id: self.next_id, from_state: self.current_state.clone(),
            to_state: to_state.to_string(), timestamp: now_millis(), trigger: trigger.to_string(),
        };
        self.next_id += 1;
        self.current_state = to_state.to_string();
        self.transitions.push_back(transition);
    ...
    pub fn transition_count(&self) -> u64 { self.next_id - 1 }
}
...
pub struct WorkflowChecksum {
    pub event_count: u64, pub activity_count: u64, pub timer_count: u64,
    pub child_count: u64, pub signal_count: u64, pub hash_value: u64,
}

impl WorkflowChecksum {
    pub fn compute_hash(&mut self) {
        let mut hash: u64 = 0xcbf29ce484222325; // FNV offset basis
        let data = [self.event_count, self.activity_count, self.timer_count, self.child_count, self.signal_count];
        for val in &data {
            hash ^= *val;
...
pub struct RetryPolicy {
    ...
    pub max_interval: Duration, pub max_attempts: u32,
    pub non_retryable_errors: Vec<String>,
}

impl Default for RetryPolicy {
    fn default() -> Self {
        Self { initial_interval: Duration::from_secs(1), backoff_coefficient: 2.0, max_interval: Duration::from_secs(100), max_attempts: 3, non_retryable_errors: Vec::new() }
    }
}

#[derive(Debug, Clone, Default)]
pub struct RetryState {
    pub current_attempt: u32, pub last_failure_reason: Option<String>,
    pub last_failure_time: Option<i64>, pub next_retry_time: Option<i64>,
    pub retry_count: u32,
}
...
impl RetryState {
    ...
    pub fn compute_next_backoff(&self, policy: &RetryPolicy) -> Duration {
        let base = policy.initial_interval.as_secs_f64() * policy.backoff_coefficient.powi(self.retry_count as i32);
...
pub enum SearchAttributeValue {
    Text(String), Keyword(String), Int(i64), Double(f64), Bool(bool),
    Datetime(i64), KeywordList(Vec<String>),
}
...
pub struct GeneratedTask {
    pub task_type: String, pub namespace_id: String, pub workflow_id: String,
    pub run_id: String, pub task_queue: String, pub priority: u32,
    pub scheduled_time: i64, pub attributes: HashMap<String, String>,
}
...
pub struct TaskGeneratorStats {
    pub tasks_generated: AtomicU64, pub transfer_tasks: AtomicU64,
    pub timer_tasks: AtomicU64, pub visibility_tasks: AtomicU64,
    pub archival_tasks: AtomicU64,
}

impl TaskGenerator {
    pub fn new(namespace_id: &str, workflow_id: &str, run_id: &str) -> Self {
        Self { namespace_id: namespace_id.to_string(), workflow_id: workflow_id.to_string(), run_id: run_id.to_string(), stats: TaskGeneratorStats::default() }
    }
    ...
    pub fn generate_workflow_task(&self, task_type: &str, task_queue: &str) -> GeneratedTask {
        ...
        GeneratedTask { task_type: task_type.to_string(), namespace_id: self.namespace_id.clone(), workflow_id: self.workflow_id.clone(), run_id: self.run_id.clone(), task_queue: task_queue.to_string(), priority: 0, scheduled_time: now_millis(), attributes: HashMap::new() }
    }
    ...
    pub fn generate_timer_task(&self, timer_id: &str, fire_time: i64) -> GeneratedTask {
        ...
        self.stats.tasks_generated.fetch_add(1, Ordering::Relaxed);
        GeneratedTask { task_type: "timer".into(), namespace_id: self.namespace_id.clone(), workflow_id: self.workflow_id.clone(), run_id: self.run_id.clone(), task_queue: String::new(), priority: 0, scheduled_time: fire_time, attributes: [("timer_id".into(), timer_id.into())].into() }
    }

    pub fn generate_visibility_task(&self, task_type: &str) -> GeneratedTask {
        self.stats.visibility_tasks.fetch_add(1, Ordering::Relaxed);
        self.stats.tasks_generated.fetch_add(1, Ordering::Relaxed);
        GeneratedTask { task_type: task_type.into(), namespace_id: self.namespace_id.clone(), workflow_id: self.workflow_id.clone(), run_id: self.run_id.clone(), task_queue: String::new(), priority: 0, scheduled_time: now_millis(), attributes: HashMap::new() }
    }

    pub fn generate_archival_task(&self) -> GeneratedTask {
        self.stats.archival_tasks.fetch_add(1, Ordering::Relaxed);
        self.stats.tasks_generated.fetch_add(1, Ordering::Relaxed);
        GeneratedTask { task_type: "archival".into(), namespace_id: self.namespace_id.clone(), workflow_id: self.workflow_id.clone(), run_id: self.run_id.clone(), task_queue: String::new(), priority: 0, scheduled_time: now_millis(), attributes: HashMap::new() }
    }
}
...
mod tests {
    ...
    fn test_activity_management() {
        ...
        assert_eq!(ms.get_activity("act-1").unwrap().state, ActivityStateEnum::Started);
        ms.record_activity_completed("act-1", vec![1, 2, 3]).unwrap();
        assert_eq!(ms.get_activity("act-1").unwrap().state, ActivityStateEnum::Completed);
    ...
    fn test_activity_retry() {
        ...
        ms.record_activity_started("act-1").unwrap();
        let will_retry = ms.record_activity_failed("act-1", "timeout".into()).unwrap();
        assert!(will_retry);
        assert_eq!(ms.get_activity("act-1").unwrap().state, ActivityStateEnum::Scheduled);
    ...
    fn test_child_workflow() {
        let ms = make_state();
        ms.add_child_workflow("child-1", "ChildWorkflow", "ns-1").unwrap();
    ...
    fn test_checksum_verification() {
        ...
        c1.event_count = 10; c1.activity_count = 2;
        c1.compute_hash();
        let mut c2 = c1.clone();
        assert!(c1.verify(&c2));
        c2.event_count = 11;
    ...
    fn test_retry_policy() {
        ...
        let state = RetryState { current_attempt: 1, retry_count: 1, ..Default::default() };
    ...
    fn test_retry_exhausted() {
        ...
        let state = RetryState { current_attempt: 3, retry_count: 3, ..Default::default() };
        assert!(!state.should_retry(&policy));
    }
    ...
    fn test_search_attributes() {
        ...
        assert!(matches!(ms.get_search_attribute("env"), Some(SearchAttributeValue::Keyword(_))));
        assert!(matches!(ms.get_search_attribute("count"), Some(SearchAttributeValue::Int(42))));
    ...
    fn test_event_appending() {
        let ms = make_state();
        let event = HistoryEvent { event_id: 0, event_type: "WorkflowExecutionStarted".into(), timestamp: now_millis(), version: 0, task_id: 1, attributes: HashMap::new() };
    ...
    fn test_buffered_events() {
        let ms = make_state();
        ms.buffer_event(HistoryEvent { event_id: 0, event_type: "SignalReceived".into(), timestamp: 0, version: 0, task_id: 0, attributes: HashMap::new() });
        ms.buffer_event(HistoryEvent { event_id: 0, event_type: "SignalReceived".into(), timestamp: 0, version: 0, task_id: 0, attributes: HashMap::new() });
