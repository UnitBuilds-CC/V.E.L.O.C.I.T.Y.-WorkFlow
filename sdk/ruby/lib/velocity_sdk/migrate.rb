...
module VelocitySDK
  module Migrate
    ...
    TEMPORAL_PATTERNS = [
      ...
      MigrationPattern.new('temporal-execute-child-workflow',
        /Workflow\.execute_child_workflow\s*\(/,
        'ctx.execute_child_workflow('),
      MigrationPattern.new('temporal-child-workflow-options',
        /ChildWorkflowOptions\.new\s*\(/,
        'ChildWorkflowOptions.new('),
      MigrationPattern.new('temporal-child-workflow-future',
        /ChildWorkflowFuture/,
        'ChildWorkflowFuture'),
      # ─── Activity Options Patterns ───────────────────────────────────────────
      MigrationPattern.new('temporal-activity-options',
        /ActivityOptions\.new\s*\(/,
        'ActivityOptions.new('),
      MigrationPattern.new('temporal-execute-local-activity',
        /Workflow\.execute_local_activity\s*\(/,
        'ctx.execute_local_activity('),
      MigrationPattern.new('temporal-local-activity-options',
        /LocalActivityOptions\.new\s*\(/,
        'LocalActivityOptions.new('),
      # ─── Coroutine & Concurrency Patterns ────────────────────────────────────
      MigrationPattern.new('temporal-async-run',
        /Async\.run\s*\(/,
        'ctx.async_run('),
      MigrationPattern.new('temporal-workflow-await',
        /Workflow\.await\s*\(/,
        'ctx.await('),
      MigrationPattern.new('temporal-workflow-await-with-timeout',
        /Workflow\.await_with_timeout\s*\(/,
        'ctx.await_with_timeout('),
      MigrationPattern.new('temporal-future',
        /Future\[/,
        'Future['),
      # ─── Relay/Nexus Operation Patterns ──────────────────────────────────────
      MigrationPattern.new('temporal-new-nexus-client',
        /Workflow\.new_nexus_client\s*\(/,
        'ctx.new_relay_client('),
      MigrationPattern.new('temporal-nexus-execute-operation',
        /nexus_client\.execute_operation\s*\(/,
        'relay_client.execute('),
      MigrationPattern.new('temporal-nexus-operation-options',
        /NexusOperationOptions\.new\s*\(/,
        'RelayOperationOptions.new('),
      # ─── Activity Context Patterns ───────────────────────────────────────────
      MigrationPattern.new('temporal-activity-get-info',
        /Activity\.info\s*\(/,
        'ctx.info('),
      MigrationPattern.new('temporal-activity-record-heartbeat',
        /Activity\.heartbeat\s*\(/,
        'ctx.heartbeat('),
      # ─── Workflow Context Patterns ───────────────────────────────────────────
      MigrationPattern.new('temporal-workflow-get-info',
        /Workflow\.info\s*\(/,
        'ctx.workflow_info('),
      MigrationPattern.new('temporal-workflow-get-logger',
        /Workflow\.logger\s*\(/,
        'ctx.logger('),
      MigrationPattern.new('temporal-workflow-with-cancel',
        /Workflow\.with_cancel\s*\(/,
        'ctx.with_cancel('),
      MigrationPattern.new('temporal-signal-external-workflow',
        /Workflow\.signal_external_workflow\s*\(/,
        'ctx.signal_external_workflow('),
      MigrationPattern.new('temporal-workflow-get-version',
        /Workflow\.get_version\s*\(/,
        'ctx.get_version('),
      MigrationPattern.new('temporal-upsert-search-attributes',
        /Workflow\.upsert_search_attributes\s*\(/,
        'ctx.upsert_search_attributes('),
      MigrationPattern.new('temporal-upsert-memo',
        /Workflow\.upsert_memo\s*\(/,
        'ctx.upsert_memo('),
      # ─── Error Handling Patterns ─────────────────────────────────────────────
      MigrationPattern.new('temporal-new-application-error',
        /ApplicationError\.new\s*\(/,
        'VelocityApplicationError.new('),
      MigrationPattern.new('temporal-canceled-error',
        /CanceledError/,
        'VelocityCanceledError'),
      MigrationPattern.new('temporal-import-nexus-package',
        /require\s+['"]temporal\/nexus['"]/,
        "require 'velocity_sdk/relay'"),
    ].freeze
