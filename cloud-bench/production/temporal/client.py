#!/usr/bin/env python3
"""
Temporal Production Benchmark Client — measures real throughput via HTTP.

Sends requests to the Temporal service endpoints and measures:
  - ops/sec (throughput)
...
parser = argparse.ArgumentParser(description="Temporal Production Benchmark Client")
parser.add_argument("profile", nargs="?", default="standard",
                    help="Benchmark profile: smoke, short, standard, stress")
parser.add_argument("--base-url", default=None,
                    help="Temporal service base URL (default: from TEMPORAL_HTTP_PORT env or http://localhost:8080)")
parser.add_argument("--output", "-o", default=None,
                    help="Output file path (default: /tmp/temporal_bench_results.json)")
_cli_args = parser.parse_args()

HTTP_PORT = int(os.environ.get("TEMPORAL_HTTP_PORT", "8080"))
BASE_URL = _cli_args.base_url or f"http://localhost:{HTTP_PORT}"
OUTPUT_PATH = _cli_args.output or "/tmp/temporal_bench_results.json"
...
async def run_workload(name, url, payload, count, concurrency=1, timeout_sec=300):
...
async def run_all_benchmarks(profile="standard"):
    ...
    # Temporal workloads have higher per-op overhead due to event sourcing.
    # Use smaller counts to avoid excessive test times.
    workloads = [
        # simple_workflow: 10 activity steps
        (
            "simple_workflow",
        ...
        # signal_storm: N signals via workflow.signal()
        (
            "signal_storm",
        ...
        # stateful: 5 activity steps with state
        (
            "stateful",
...
async def main():
    ...
    print(f"=== Temporal Production Benchmark (profile: {profile}) ===")
    ...
    try:
        ...
        print(f"ERROR: Cannot connect to Temporal service: {e}")
        sys.exit(1)
